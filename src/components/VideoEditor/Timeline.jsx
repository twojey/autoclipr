...
const Timeline = ({
  ...
  // États
  const [resizing, setResizing] = useState(null); // 'start' ou 'end' ou null
  ...
  const visibleTimeRange = useRef({ start: 0, end: duration });
  ...
  const handleWindowDrag = useCallback((e, info) => {
  ...
  const handleTimelineClick = useCallback((e) => {
    if (resizing || isDraggingWindow) return;
